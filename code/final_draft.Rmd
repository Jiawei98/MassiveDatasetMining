...
To get an overall insight into the dataset and methodology feasibility, we performed our workflow here (Figure 1).

![Workflow](../figures/workflow.png)

Detailly, we first pre-pressed the image on CHTC to get dimensional reduced, grayscale image for model training. After that, we split the set into training set (60%), validation set (30%), and test set(10%). We use parallel computation on CHTC to fit multiple CNN models under LeNet5 architecture with different hyperparameters on the training set to get an insight into what the hyperparameters should be used, and prepare for the larger scale hyperparameter tuning in the future.

# Data Description

The dataset is sourced from Kaggle, which could be downloaded from [here](http://www.kaggle.com/paramaggarwal/fashion-product-images-dataset). 
...
Both R and Python are used for different purposes in this project. R is installed with package OpenImageR to pre-process the images and Python is installed with several essential packages (numpy, torch, pandas, torchvision, PIL) to build a neural network. Therefore, two interactive jobs were created to first download necessary files, then to install packages and finally return built files for R and Python respectively. We have learned about how to build R environment in class. To build up the Python environment, we followed the [CHTC Python instruction](https://chtc.cs.wisc.edu/python-jobs.shtml). More details can be found in the illustration below (Figure 2):

![Building Python Environment](../figures/CHTC_Env.png){width=50%}
...
1. **Grayscale**  
Since we will be mostly deal with the shape of the clothing, we first convert the colored images with 3 RGB channels into black and white images with only one single channel. Through function:
$$Y_{Linear} = 0.2126*R_{Linear}+0.7152*G_{Linear}+0.0722B_{Linear}$$

2. **Dimensional reduction**  
Original number of pixels of an image is beyond the computational power of PC and even hard for high-performance clusters. Thus, we reduced the dimension of the images. We decided on $32 \times 32$, because it contains sufficient information about the shape and is a reasonable number of pixels to fit models.

To complete image pre-processing efficiently, we divided and compressed all 44440 images into 59 .tgz files, then used CHTC to process these 59 .tgz files in parallel. More details can be found in the illustration below (Figure 3):

![Images Pre-processing](../figures/CHTC_Cvt_Img.png){width=50%}
...
More detailly, we built a CNN model with the structures of 2 Convolution Layer, 2 Max-Pooling Layer, and 5 FC Layer with LeNet rule, then ran parallel jobs on CHTC with the random selected hyperparameters from the table and compare their accuracy on validation set. Finally, we got a summarization of the performance for different hyperparameters.
...
In the second Convolution Layer, with a Kernel Size = $N_2$ and Stride Length = 1, we got $N_2$ channels of $16 \times 16$ matrix through valid padding. The second Max-Pooling Layer is applied after that to finalize our dataset to be $8 \times 8 \times N_2$. 
...
In this part, we did hyperparameter tuning by random search. In our first draft, we only compared 600 hyperparameter combinations. In our final draft, we adjusted the range of hyperparameters based on previous result and compared 3000 hyperparameter combinations.

The following figure illustrate how we get all accuracy for 3000 hyperparameter combinations in parallel by using CHTC (Figure 4).

![Hyperparameter tuning](../figures/CHTC_CNN.png){width=50%}

More details will be introduced in the following.

## First Try

After building up the CNN model in Python, we used CHTC to do hyperparameter tuning efficiently. The following table shows the range of each hyperparameter in our first draft:
...
This time, we randomly generated 3000 different combinations of hyperparameters. Then, we submitted 3000 parallel jobs on CHTC to fit 3000 CNN models (epoch_num=15) with different combination of hyperparameters. More details can be found in the illustration below:
...
Note that: if we focus on models with validation accuracy larger than 88% and order them by running time, we could find the model with highest training accuracy also took the longest training time among them. Therefore, if we'd like to consider trade-off of time, it's also a good idea to choose another model with validation accuracy 88.00% and training time 1168s. The corresponding hyperparameters combination is filter_num1=12, kernel_size1=3, filter_num2=64, kernel_size2=3, hidden_layer1=120, hidden_layer2=60, learning_rate=0.002 and batch_size=256. In this way, we can save about 2/3 of the training time.
...
Based on these plots, we can find 8 hyperparameters with highest average validation accuracy. However, compared with our final models, the combination of these hyperparameters leads to a lower validation accuracy 87.51%. Therefore, the relationship among hyperparameters and validation accuracy is very complex. This confirms the necessity of random search in hyperparameter tuning.


## Final Model

By using CHTC, we figured out the best combination of hyperparameters among 3000 candidates efficiently. Then, We fitted a final model with data in both training and validation sets based on these hyperparameters, and use the **"secret"** test dataset to evaluate the perfermance of this model.
...
abline(h=0.88140, col="gray", lty=3, lwd=1.5)
...
The accuracy plot showed that: As the number of epoch increasing, the training accuracy kept increasing and was approaching to 100%, while the test accuracy also kept increasing at first but stabilized at about 88%. It proved that our final model has a good performance on product images classification.

# Conclusion

In this project, we build a classifier based on CNN, which can predict the category of a product by using its image. By using CHTC, we boost up our pipeline at 2 steps: 1. data preprocessing for more than 40 thousand images. 2. hyperparameter tuning among 3 thousand combinations. The tasks in each step are independent and time-consuming, so there is no doubt that parallel computing is a great method to save our time. The accuracy of our final model is about 

Since we are not very proficient in CNN, there might be some details in model design that can be improved. However, we still think this is a wonderful practice for us to get familiar with CHTC and parallel computing.

\newpage

# Reference

1. Hutter F, Hoos H, Leyton-Brown K. An efficient approach for assessing hyperparameter importance[C]//International conference on machine learning. PMLR, 2014: 754-762.

2. Aszemi N M, Dominic P D D. Hyperparameter Optimization in Convolutional Neural Network using Genetic Algorithms[J].

3. CHTC Official Website: Running Python Jobs on CHTC: https://chtc.cs.wisc.edu/python-jobs.shtml
