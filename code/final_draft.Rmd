...
More detailly, in the first draft, we build a CNN model with the structures of 2 Convolution Layer, 2 Max-Pooling Layer, and 5 FC Layer with LeNet rule, run 600 parallel jobs on CHTC with the random selected hyperparameters from the table above, compare their accuracy on validation set, and finally get a summarization of the performance for different hyperparameters.
...
```{r, echo=TRUE, figures-side, fig.show="hold", out.width="45%", fig.align = "center"}

library(ggplot2)
acc = read.csv("res_table.csv")
#colnames(acc) = c("filter_number_1","filter_number_2","learning_rate","batch_size","train_accuracy","validation_accuracy","time/s")
lrate = tapply(acc$`validation_accuracy`, acc$learning_rate,mean)
lrate = data.frame(learning_rate = factor(names(lrate)),
...
lrate = data.frame(batch_size = factor(names(lrate),levels = c("32","64","128","256")),
                   validation_accuracy = lrate,group="")
...
lrate = tapply(acc$validation_accuracy, acc$filter_num1 ,mean)
lrate = data.frame(filter_num1 = factor(names(lrate),levels = c("4","8","12")),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=filter_num1,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9]) + ylim(40,70) +
        theme_minimal() + 
...
lrate = tapply(acc$validation_accuracy, acc$filter_num2 ,mean)
lrate = data.frame(filter_num2 = factor(names(lrate),levels = c("24","32","48")),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=filter_num2,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9]) + ylim(40,70) +
        theme_minimal() + 
...
# Final draft work (newly appended)

Our results from the first draft gives us a reference to preform the second hyperparameter tuning. With the experience for the first time, we could reset the testing parameters into a more reasonable scale. We performed ANOVA to the hyperparameter setting, with the consideration of all 2nd level interactions, to define which hyperparamters have a significant influence on the accuracy rate. with Bonferroni correction,we extracted all terms with $p < 0.05/37$, which are **Filter Numeber 1**, **Filter Numeber 2**, **Learning Rate** and **Batch Size**.

```{r}
acc = read.csv("res_table.csv")
acc[,2:9] = apply(acc[,2:9],2,factor)
fit1 = lm(validation_accuracy~(.-id-train_accuracy)^2,data=acc)
aov1 = anova(fit1)
head(aov1)
rownames(aov1)[aov1$`Pr(>F)` < 0.05/37]
```

With the results from first attempt, we reset the parameters to new values in the following table. which could be assigned into 384 combinations. We rerun the all the combinations to get the best result...

| Hyperparameter | Annocatation | Value |
|---|:---:|:---:|
| Filter Numeber 1 | $N_1$  | [3,4,5,6] |
| Kernel  Size 1 | $K_1$ | 3 |
| Filter Numeber 2 | $N_1$ | [8,12,16,20,24,28] |
| Kernel  Size 2 | $K_2$ | 3 |
| Hidden Layer 1 | $H_1$ | 120 |
| Hidden Layer 2 | $H_2$ | 40 |
| Learning Rate | $R$ | [0.002,0.001,0.0005,0.0001] |
| Batch Size | $S$ | [64,128,256,512] |


```{r, fig.show="hold", out.width="45%", fig.align = "center"}
res <- fit1$residuals
qplot(sample=fit1$residuals,data=acc)+labs(title="QQplot of validation accuracy", y="sample",x="theoritical")+
  geom_qq_line()
#plot(fit1$fitted.values, rstandard(fit1),ylim=c(-4,4),ylab = "standardized residual",xlab = "predicted accuracy",main = "standardized residual plot")
```
We tried to diagnose normal model assumption by a qq plot. Here we would say the normal assumption is plausible, while there seems to be more data located at the extremes of the distribution. 

```{r, echo=TRUE, figures-side1, fig.show="hold", out.width="45%", fig.align = "center"}

library(ggplot2)
acc = read.table("CNN/all_res.csv",sep=",",header = TRUE)
#colnames(acc) = c("filter_number_1","filter_number_2","learning_rate","batch_size","train_accuracy","validation_accuracy","time/s")
options(scipen = 200)
lrate = tapply(acc$`validation_accuracy`, acc$learning_rate,mean)
lrate = data.frame(learning_rate = names(lrate),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=learning_rate,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9]) + 
...
lrate = tapply(acc$`validation_accuracy`, acc$batch_size ,mean)
lrate = data.frame(batch_size = as.factor(c(64,128,256,512)),
                   validation_accuracy = lrate,group=1)
ggplot(lrate,aes(x=batch_size,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9])  +
        theme_minimal() + 
...
lrate = tapply(acc$validation_accuracy, acc$filter_number_1 ,mean)
lrate = data.frame(filter_num1 = as.factor(c(3,4,5,6)),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=filter_num1,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9])  +
...
lrate = tapply(acc$validation_accuracy, acc$filter_number_2 ,mean)
lrate = data.frame(filter_num1 = as.factor(c(8,12,16,20,24,28)),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=filter_num1,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9])  +
      ...
      theme(
            ...
            axis.title = element_text(size=14),
            legend.title = element_text(size=14,face="bold"))
acc[acc$validation_accuracy==max(acc$validation_accuracy),]
test<-acc[acc$validation_accuracy>86,-5]
head(test[order(test$time),])
acc[acc$filter_number_1==5 & acc$filter_number_2==28 & acc$learning_rate==0.002 & acc$batch_size==64,]
```


The highest validation accuracy in all 384 models is 86.98%, with time 706.71s. The corresponding hyperparameters combination is filter_num1=6, filter_num2=28, learning_rate=0.001 and batch_size=64, with kernel_size1=3, kernel_size2=3, hidden_layer1=120, hidden_layer2=40. However, if we focus on models with validation accuracy larger than 86 and order them by running time, we could find models with great improvement in running time. So if we'd like to consider trade-off of time, we could choose the model with four hyperparameters combination to be filter_num1=5, filter_num2=24, learning_rate=0.002 and batch_size=128, with validation accuracy to be 86.40% and time to be 510.80s. The reduce in time is about 27.72%, while the accuracy only drops 0.58%. We’ll use these hyperparameters to fit a new model with data in both training and validation sets, and then use the “secret” test dataset to evaluate the perfermance of this model.


```{r, echo=TRUE, figures-side2, fig.show="hold", out.width="45%", fig.align = "center"}
options(scipen = 200)
lrate = tapply(acc$time, acc$learning_rate,mean)
lrate = data.frame(learning_rate = names(lrate),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=learning_rate,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9]) + 
        theme_minimal() + 
      labs(title="Scatterplot for running time on Learning Rate",
           x="Learning Rate",
           y="Average Time") +
      theme(
            plot.title = element_text(size = 18,hjust = 0.5),
...
lrate = tapply(acc$time, acc$batch_size ,mean)
lrate = data.frame(batch_size = as.factor(c(64,128,256,512)),
                   validation_accuracy = lrate,group=1)
ggplot(lrate,aes(x=batch_size,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9])  +
        theme_minimal() + 
            labs(title="Scatterplot for running time on Batch Size",
           x="Batch Size",
           y="Average Time") +
      theme(
            plot.title = element_text(size = 18,hjust = 0.5),
...
lrate = tapply(acc$time, acc$filter_number_1 ,mean)
lrate = data.frame(filter_num1 = as.factor(c(3,4,5,6)),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=filter_num1,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9])  +
        theme_minimal() + 
            labs(title="Scatterplot for running time on filter number 1",
           x="filter number 1",
           y="Average Time") +
      theme(
            plot.title = element_text(size = 18,hjust = 0.5),
...
lrate = tapply(acc$time, acc$filter_number_2 ,mean)
lrate = data.frame(filter_num1 = as.factor(c(8,12,16,20,24,28)),
                   validation_accuracy = lrate,group="")
ggplot(lrate,aes(x=filter_num1,y=validation_accuracy)) + 
  geom_line(aes(group = group),color=blues9[3],size=2) +
  geom_point(size=4,color=blues9[9])  +
        theme_minimal() + 
            labs(title="Scatterplot for running time on filter number 2",
           x="filter number 2",
           y="Average Time") +
      theme(
            plot.title = element_text(size = 18,hjust = 0.5),
...
test<-acc[acc$validation_accuracy>86,-5]
head(test[order(test$time),])
acc[acc$filter_number_1==3 & acc$filter_number_2==8 & acc$learning_rate==0.001 & acc$batch_size==256,]
acc[acc$time==min(acc$time),]
```

Based on these plots, we could always find the best hyperparameters for validation accuracy and running time. Filter number 1=5, filter number 2=28, learning rate=0.002 and batch size=64 give the highest average validation accuracy separately; Filter number 1=3, filter number 2=8, learning rate=0.001 and batch size=256 give the lowest average running time separately. However, compared with our final models, the combination of best hyperparameters not always gives the best results. Also, if we choose combination of best hyperparameters for validation accuracy: filter number 1=5, filter number 2=28, learning rate=0.002 and batch size=64, the accuracy is 86.02%. Therefore, the relationship among hyperparameters and validation accuracy/time is not convex.
