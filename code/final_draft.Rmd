...
![Building Python Environment](../figures/CHTC_Env.png){width=60%}
...
Original number of pixels for an image is beyond the computational power of PC and even hard for high-performance clusters. Thus, we reduced the dimension of the images. We decided on $32 \times 32$, because it contains sufficient information about the shape and won't lead to a huge computational burden.

To complete image preprocessing efficiently, we divided and compressed all images into 59 .tgz files, then used CHTC to process these 59 .tgz files in parallel. More details can be found in the illustration below (Figure 3):

![Images Preprocessing](../figures/CHTC_Cvt_Img.png){width=60%}

After that, we split all 32618 images into training set (60%), validation set (30%), and test set(10%).
...
We get an insight into the importance of hyperparameters through ANOVA, assigning each hyperparameter as an effect.[3] With the main effects and second level interactions included, we got 36 effects and evaluated their significance through Bonferroni Correction, that is, extracting all terms with $p < 0.05/36 = 0.0013$ as significant. It turns out that 18 out of 36 effects are significant, suggesting our hyperparameter tuning is meaningful. Therefore, we decided to perform a second hyperparameter tuning on finer and more reasonable divisions and larger intervals.
...
By using CHTC, we figured out the best combination of hyperparameters among 3000 candidates efficiently. Then, We fitted a final model with data in both training and validation sets based on these hyperparameters, and use the test dataset has not been used before to evaluate the perfermance of this model.
