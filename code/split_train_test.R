...
image_info<-image_info[,c("id","subCategory")]
image_category_count<-table(image_info$subCategory)
image_category_names<-names(image_category_count[image_category_count>10])
image_info<-image_info[image_info$subCategory %in% image_category_names, ]
image_info<-image_info[order(image_info$subCategory),]
...
for (cur_num in 1:nrow(image_info)){
  ...
  category_name<-image_info$subCategory[cur_num]
